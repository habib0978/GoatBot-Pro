...
async function sendOwnerNotice(api, message) {
  try {
    ...
    const ownerID = adminBot[0] || OWNER_ID;
...
module.exports = {
  config: {
    name: "lock",
    version: "3.0.0",
    author: "🔰 RAHAT ISLAM - Upgraded",
    countDown: 3,
    role: 1, // Only group administrators/bot administrators
    shortDescription: "Advanced persistent group name, emoji and image protection",
    category: "box",
    guide: {
      en: "lock [on | off | status | name <name> | emoji <emoji> | reply photo + lock image]"
    }
  },
  ...
  onEvent: async ({ api, event }) => {
    ...
    try {
      ...
      if (isAdmin(threadInfo, senderID)) return;
